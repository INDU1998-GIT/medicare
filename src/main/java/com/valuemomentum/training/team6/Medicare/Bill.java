...
public class Bill {
    ...
    void newBill() {
    	...
    	stmt  =con.prepareStatement("insert into Bills(P_id,appointmentfee,NoofDays,testname,Scanfee) values(?,?,?,?,?)");
    ...
    void UpdateBill() {
    	...
    	stmt  =con.prepareStatement( "UPDATE Bills SET appointmentfee=?,NoofDays=?,testname=?,Scanfee=? WHERE P_id=?");
    ...
    void DeleteBill() {
        try {
              ...
              int Pid=s.nextInt();
              String query="Delete  from Bills where P_id=?";
    ...
    void printBill()
    {
    	 try {
             ...
             System.out.println("Enter patient id:");
             int id=s.nextInt();
             String query = "select P_id,appointmentfee,NoofDays,Scanfee,(NoofDays*appointmentfee)+Scanfee as Totalbill from Bills where P_id = ?";
             stmt = con.prepareStatement(query);
             stmt.setInt(1, id);
